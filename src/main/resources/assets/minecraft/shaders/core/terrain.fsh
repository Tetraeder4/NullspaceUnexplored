...
// Simple 3x3 Box Blur function
vec4 applyBlur(sampler2D tex, vec2 uv, vec2 texelSize, float blurAmount) {
    ...
    for (int x = -1; x <= 1; x++) {
        for (int y = -1; y <= 1; y++) {
            ...
            // Sample base color using standard texture lookup for neighbor blur
            vec4 sampleCol = texture(tex, uv + offset);
            totalColor += sampleCol;
            totalWeight += 1.0;
        }
    }
...
void main() {
    // 1. Fetch base texture sample using RGSS or Nearest filtering
    vec4 color = (UseRgss == 1 ? sampleRGSS(Sampler0, texCoord0, 1.0f / TextureSize) : sampleNearest(Sampler0, texCoord0, 1.0f / TextureSize)) * vertexColor;

    // 3. Alpha cutoff check
    #ifdef ALPHA_CUTOUT
